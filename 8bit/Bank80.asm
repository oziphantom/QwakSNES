...
InitSNESAndMirror	; this is defualt init sequence
	...
	ldx #len(TitleScreenData.AllStrings)-1
	stx ZPTemp
-	ldx ZPTemp
	...
	lda #%10000001
	sta $4200		; enable VBlank NMI and enable auto Joypad reading

	; set the main FSM to point to the title screen FSM
	lda #<titleScreenLoop
	sta GameStatePointer.lo
	lda #>titleScreenLoop
	sta GameStatePointer.hi
...
;	stz PasswordEntryIndex
	...
	ldx #(4*4)-1							; 4 sprites at 4 bytes each
...
_ASSERT_axy8
	...
	sta GameStatePointer.lo
	lda #>GAMELOOP
	sta GameStatePointer.hi				; set main FSM to game loop
...
;	_bossLevel
	...
	sta GameStatePointer.lo
	lda #>GAMELOOP
	sta GameStatePointer.hi							; set the main FSM to Gameloop
...
_ASSERT_axy8
	; print string
	ldx #len(TitleScreenData.AllStrings)	; this doesn't have Game Over in it, which is added after
...
-	sta GameData.score,x
	...
	;
_higher
	ldx #size(sGameData.score)-1	; save the current score into the high score
...
_ASSERT_axy8
	...
	bne _exit									; aka fire released
		...
		beq _exit
			...
			sta GameStatePointer.lo
			lda #>titleScreenLoop
			sta GameStatePointer.hi			; move main FSM to titlescreen
_exit
	rts
...
_ASSERT_jsr
_ASSERT_axy8
; so the wonderful world of clipping an 8 bit value in 8bits
; you can't just check overflow because you add negative numbers
; you could check if you are doing a negative value, negaite it and
; subtract it and check over/underflow that way. 
; firstly sprites have a width and must remain on screen, although some sprites are
; 24 pixels wide I treat all as if they are 16. Which on a C64 was an issue as the HUD
; was on the side. On a SNES no problem ;) 
; if you are very low and suddenly get very high you overflow.
; we don't actually clip to 256 we clip to 256-width of sprite which is 16 so as long as things
; can't move more than 16px a frame we are fine, we just check if you are under 256-16 and clip if over
; as long as we where on that side of the screen to begin with.
; I do a mid range check for the "don't care just do it" case. It saves some clocks.
	stz DidClipX										; not clipped yet
	lda mplexBuffer.xpos,x							; read the X
	cmp #256-32
	bcs _veryRight										; if in the right most 32 pixels we are very right
	cmp #32
	bcs _justAddAndGo									; if over 32 we are in the middle
	;_veryLeft
		;clc												; just did a bcs so c must be clear
		adc checkSpriteToCharData.xDeltaCheck
		cmp #200
		bcc _justStore									; if we are < 200 we are still on the correct side
			inc DidCLipX								; we are over, clip
			lda #0
			bra _justStore
			;
	_veryRight
		clc												; so we are in the last 2 "blocks"
		adc checkSpriteToCharData.xDeltaCheck
		cmp #256-16										; we don't want to go past the edge of the last block
		bcc _justStore
			inc DidClipX								; we are over clip
			lda #256-16
			bra _justStore
	_justAddAndGo
		clc
		adc checkSpriteToCharData.xDeltaCheck	; safe area just add and go
	_justStore
...
_ASSERT_axy8
		...
		ldx LevelData.levelGraphicsSet		; this is the "current levels" set at this point
		lda WallCharLUT.lo,x						; set the DMA source address based upon the set we want
		sta $804302
		lda WallCharLUT.hi,x
		sta $804303
		lda #`BackShadowChars
		sta $804304
		#XY16											; DMA 16bit to avoid the pain ;) 
...
psaaixCommon
	...
	sta $802181
	lda TitleScreenData.stringPos.hi,x	; load up string X's Screeen Mirror pos
	sta $802182
	ldy TitleScreenData.string,x
	lda StringTableLUTLo,y					; load up the strings data src pointer
	sta Pointer1.lo
...
_bulletTile
	...
	lda PlayerBulletAnimData.lo,x
	sta OAMMirror+10
	lda PlayerBulletAnimData.hi,x
	sta OAMMirror+11
...
_ASSERT_axy8
		...
		sta Pointer1.lo
		lda #>tileMapTemp
		sta Pointer1.hi				; pointer 1 holds pointer to the active level data
		lda #<ScreenMirror
		sta ZPLong.lo
		lda #>ScreenMirror
		sta ZPLong.hi
	...
	_pltX		lda (Pointer1),y		; tile num
				...
				lda ZPLong.lo
				adc #4
				sta ZPLong.lo			; advance screen pointer 2 tiles to the right
				bcc +
					inc ZPLong.hi		; this is safe to do as 16bit maths
	...
	+	pla								; restore the rows counter
		...
		pha								; save it back on the stack
			clc
			lda ZPLong.lo				; we have to offset the screen position to the next "row"
			adc #128-64					; each row is 32 chars, which is words so 64 bytes
			sta ZPLong.lo				; and we have 2 rows which is 128 bytes, we are already 1 row over
			bcc +							; so 128-64 to get to the start of the next tile row
				inc ZPLong.hi
	+		gra _pltY
_exit
	rts
...
_ASSERT_axy8
		...
		stz Pointer4.hi
		asl Pointer4.lo
		rol Pointer4.hi	; 16bit multiply 
		asl Pointer4.lo	; tile num x 4
		rol Pointer4.hi
		...
		lda Pointer4.hi
		adc #>fileTiles
		sta Pointer4.hi	; add the start of the map -> chars LUT table
		lda (Pointer4)		; read 1st char
		sta [ZPLong]		; lo byte of Dest Char 1
		lda #0
		ldy #1
		sta [ZPLong],y		; hi byte of Dest Char 1
		lda (Pointer4),y	; read 2nd char
		ldy #2
		sta [ZPLong],y		; lo byte of Dest Char 2
		iny
		lda #0
		sta [ZPLong],y		; hi byte of Dest Char 2
		ldy #2
		lda (Pointer4),y	; read 3rd char
		ldy #64
		sta [ZPLong],y		; lo byte of Dest Char 3
		iny
		lda #0
		sta [ZPLong],y		; hi byte of Dest Char 3
		ldy #3
		lda (Pointer4),y	; read 4th char
		ldy #66
		sta [ZPLong],y		; lo byte of Dest Char 4
		iny
		lda #0
		sta [ZPLong],y		; hi byte of Dest Char 4
...
_ASSERT_axy8
	...
	sta TempX
	#a16			; since this is fresh SNES code I've made it in 16bit from the get go
					; to see how this would be done in 8bit look at function above
...
_ASSERT_JSR
		...
		sta LevelData.exitIndex+1					; and I don't know where the exits are
		lda #<tileMapTemp
		sta Pointer1.lo
		lda #>tileMapTemp
		sta Pointer1.hi								; this the location of the converted map data
		ldx GameData.currLevel
		lda LevelTableLo,x
		sta Pointer2.lo
		lda LevelTableHi,x
		sta Pointer2.hi								; this is the location of the raw level data
; read level pointers
		ldy #0
		...
		clc
		adc Pointer2
		sta EntityDataPointer.lo					; which is relative to the start of this "level spec"
		iny
		lda (Pointer2),y
		adc Pointer2.hi
		sta EntityDataPointer.hi					; the level data also has another unused pointer in it
		clc
		lda Pointer2.lo
		adc #4											; so we skip the Entity Data Ptr + unused pointer
		sta Pointer2.lo
		bcc +
			inc Pointer2.hi
...
_cont
		...
		bne _loop
		clc
		lda Pointer2.lo								; pMapDataSrc += 16
		adc #16
		sta Pointer2.lo
		bcc +
			inc Pointer2.hi
	+	clc
		lda Pointer1.lo								; pTempMap += 16
		adc #16
		sta Pointer1.lo
		bcc +
			inc Pointer1.hi
...
_ASSERT_jsr
_ASSERT_axy8
	lda PlayerData.yDeltaAccum.lo							; yDelta += JumpSpeed[normal/float]
	clc
	adc PlayerJumpLUT,x
	sta PlayerData.yDeltaAccum.lo
	lda PlayerData.yDeltaAccum.hi
...
_ASSERT_axy8
	...
	bne bulletExit										; if not burst
		...
		bne _bulletFull								; are we full?
			lda bulletFrame
			cmp #kSprites.bulletSprite+2			; have we reached the full size
			beq _bulletFull
...
_bulletFull
	...
	sta Pointer1.lo
	lda #>UpdateBulletEndYColl
	sta Pointer1.hi									; set post collision callback
	lda #0
	sta CollisionResult
	tay ; ldy #0
...
_exit2
	...
	;
_bossBear
	lda #BearEntAnimData[0]								; set to frame for head left
	sta EntityData.animBase,x
	lda #BearEntAnimData[1]								; set to frame for head right
	sta EntityData.animBase+1,x
	lda #BearBodyAnimData[0]							; set to frame for body left
	sta EntityData.animBase+2,x
	lda #BearBodyAnimData[1]							; set to frame for body right
	sta EntityData.animBase+3,x
	bra _nextEnt

_bossOctopuss
	lda #OctopussEntAnimData[0]						; set to frame for head left
	sta EntityData.animBase,x
	lda #OctopussEntAnimData[1]						; set to frame for head right
	sta EntityData.animBase+1,x
	lda #OctopussBodyAnimData[0]						; set to frame for body left
	sta EntityData.animBase+2,x
	lda #OctopussBodyAnimData[1]						; set to frame for body right
	sta EntityData.animBase+3,x
	bra _nextEnt
...
_ASSERT_axy8
	...
	beq _notFlash
		...
		bne _notFlash
			...
			stz EntityData.palleteOffset+1,x
			stz EntityData.palleteOffset+2,x
			stz EntityData.palleteOffset+3,x
_notFlash
	...
	beq _exit
		...
		bne _exit								; jump into body, get pushed up and Quick Kill them in one jump
			...
			beq _killedBoss
				...
				lda #kSPal_1					; next pal
				sta EntityData.palleteOffset,x
				sta EntityData.palleteOffset+1,x
				sta EntityData.palleteOffset+2,x
				sta EntityData.palleteOffset+3,x
...
string .byte AllStrings[:,0]..(kStrings.gameOver,)	; tack the Game over on the end so len(AllStrings) gives the index
